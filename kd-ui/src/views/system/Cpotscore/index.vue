  ...
  <div class="app-container">
    <el-form :model="queryParams" ref="queryForm" size="small" :inline="true" v-show="showSearch" label-width="68px">
      ...
      <!-- <el-form-item label="面部表情" prop="cpotFacialExpression">
        <el-input
          v-model="queryParams.cpotFacialExpression"
          placeholder="请输入面部表情"
          clearable
          @keyup.enter.native="handleQuery"
        />
      ...
      <!-- <el-form-item label="CPOT人机协调" prop="cpotSynchronyWithVentilator">
        <el-input
          v-model="queryParams.cpotSynchronyWithVentilator"
          placeholder="请输入CPOT人机协调"
          clearable
          @keyup.enter.native="handleQuery"
        />
      </el-form-item>
      <el-form-item label="CPOT发声" prop="cpotVocalizationWithoutTracheostomyOrIntubation">
        <el-input
          v-model="queryParams.cpotVocalizationWithoutTracheostomyOrIntubation"
          placeholder="请输入CPOT发声"
          clearable
          @keyup.enter.native="handleQuery"
        />
      </el-form-item>
      <el-form-item label="使用的镇痛药物" prop="cpotAnalgesics">
        <el-input
          v-model="queryParams.cpotAnalgesics"
          placeholder="请输入使用的镇痛药物"
          clearable
          @keyup.enter.native="handleQuery"
        />
      </el-form-item>
      ...
      <el-form-item label="CPOT肢体运动" prop="cpotLimbMovement">
        <el-input
          v-model="queryParams.cpotLimbMovement"
          placeholder="请输入CPOT肢体运动"
          clearable
          @keyup.enter.native="handleQuery"
        />
      </el-form-item>
      <el-form-item label="CPOT肌肉张力" prop="cpotMuscleTension">
        <el-input
          v-model="queryParams.cpotMuscleTension"
          placeholder="请输入CPOT肌肉张力"
          clearable
          @keyup.enter.native="handleQuery"
        />
      </el-form-item>
      <el-form-item label="CPOT总分" prop="cpotTotal">
        <el-input
          v-model="queryParams.cpotTotal"
          placeholder="请输入CPOT总分"
          clearable
          @keyup.enter.native="handleQuery"
        />
      </el-form-item> -->
    ...
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleAdd"
          v-hasPermi="['system:Cpotscore:add']"
        >新增</el-button>
      </el-col>
      <!-- <el-col :span="1.5">
        <el-button
          ...
          :disabled="single"
          @click="handleUpdate"
          v-hasPermi="['system:Cpotscore:edit']"
        >修改</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="multiple"
          @click="handleDelete"
          v-hasPermi="['system:Cpotscore:remove']"
        >删除</el-button>
      </el-col> -->
      <el-col :span="1.5">
        <el-button
      ...
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleExport"
          v-hasPermi="['system:Cpotscore:export']"
        >导出</el-button>
      </el-col>
      <right-toolbar :showSearch.sync="showSearch" @queryTable="getList"></right-toolbar>
    </el-row>

    <el-table v-loading="loading" :data="CpotscoreList" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="面部表情" align="center" prop="cpotFacialExpression" />
      <el-table-column label="CPOT人机协调" align="center" prop="cpotSynchronyWithVentilator" />
      <el-table-column label="CPOT发声" align="center" prop="cpotVocalizationWithoutTracheostomyOrIntubation" />
      <el-table-column label="使用的镇痛药物" align="center" prop="cpotAnalgesics" />
      <el-table-column label="提示信息" align="center" prop="cpotCue" />
      <el-table-column label="CPOT肢体运动" align="center" prop="cpotLimbMovement" />
      <el-table-column label="CPOT肌肉张力" align="center" prop="cpotMuscleTension" />
      <el-table-column label="CPOT总分" align="center" prop="cpotTotal" />
...
<script>
import { listCpotscore, getCpotscore, delCpotscore, addCpotscore, updateCpotscore } from "@/api/system/Cpotscore"
...
export default {
  ...
  data() {
    return {
      ...
      upload: {
        ...
        // 上传的地址
        url: process.env.VUE_APP_BASE_API + "/system/Cpotscore/importData"
      },
      // 表单校验
      rules: {
      }
  ...
  methods: {
    ...
    getList() {
      ...
      listCpotscore(this.queryParams).then(response => {
        this.CpotscoreList = response.rows
    ...
    handleAdd() {
      this.reset()
      this.open = true
      this.title = "添加重症护理疼痛观察"
    },
    /** 修改按钮操作 */
    handleUpdate(row) {
      this.reset()
      const Id = row.id || this.ids
      getCpotscore(Id).then(response => {
        this.form = response.data
        this.open = true
        this.title = "修改重症护理疼痛观察"
      })
    },
    /** 提交按钮 */
    submitForm() {
      this.$refs["form"].validate(valid => {
        if (valid) {
          if (this.form.id != null) {
            updateCpotscore(this.form).then(response => {
              this.$modal.msgSuccess("修改成功")
              this.open = false
              this.getList()
            })
          } else {
            addCpotscore(this.form).then(response => {
    ...
    handleDelete(row) {
      ...
      this.$modal.confirm('是否确认删除重症护理疼痛观察编号为"' + Ids + '"的数据项？').then(function() {
        return delCpotscore(Ids)
      }).then(() => {
        this.getList()
...
/** 下载模板操作 */
importTemplate() {
  this.download('system/Cpotscore/importTemplate', {
...
submitFileForm() {
  this.$refs.upload.submit()
    ...
    /** 导出按钮操作 */
    handleExport() {
      this.download('system/Cpotscore/export', {
        ...this.queryParams
      }, `Cpotscore_${new Date().getTime()}.xlsx`)
    }
  }
}
</script>
