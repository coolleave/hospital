<template>
  <div class="app-container">
    ...
    <el-table v-loading="loading" :data="recordList" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="执行时间" align="center" prop="Time" width="180">
        <template slot-scope="scope">
          <span>{{ parseTime(scope.row.Time, '{y}-{m}-{d}') }}</span>
        </template>
      </el-table-column>
      <el-table-column label="医疗行为" align="center" prop="instructions" />
...
export default {
  ...
  data() {
    return {
      ...
      upload: {
        ...
        // 上传的地址
        url: process.env.VUE_APP_BASE_API + "/system/record/importData"
      },
      // 表单校验
      rules: {
      }
  ...
  methods: {
    ...
    handleDelete(row) {
      ...
      this.$modal.confirm('是否确认删除行医记录编号为"' + Ids + '"的数据项？').then(function() {
...
/** 下载模板操作 */
importTemplate() {
  this.download('system/record/importTemplate', {
